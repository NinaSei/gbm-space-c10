...
md(r"""🔬 **TASK 4.3 - scVI.** scVI is a deep generative model trained on raw counts. By default you **load** a GPU-trained latent (fast). Set `TRAIN_SCVI = True` to train scVI on **your own cells** instead (uses a GPU if available; a few minutes on CPU). Either way it fills `adata.obsm['X_scvi']` for your cells, so the comparison below works the same.""")
